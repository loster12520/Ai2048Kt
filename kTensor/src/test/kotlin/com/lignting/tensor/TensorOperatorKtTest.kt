...
class TensorOperatorKtTest {
    ...
    fun testPlus() {
        ...
        assert(
            intTensorPlusDoubleTensor.data.contentEquals(
                arrayOf(
                    2.0.typeDouble(), 4.0.typeDouble(), 6.0.typeDouble()
                )
            )
        )
        assert(
            doubleTensorPlusDoubleTensor.data.contentEquals(
                arrayOf(
                    2.0.typeDouble(), 4.0.typeDouble(), 6.0.typeDouble()
                )
            )
        ...
        assert(
            doubleTensorPlusIntTensor.data.contentEquals(
                arrayOf(
                    2.0.typeDouble(), 4.0.typeDouble(), 6.0.typeDouble()
                )
            )
    ...
    fun testPlusBoardCast() {
        ...
        println("intTensorPlusIntTensor: ${intTensorPlusIntTensor.data.toList()}")
        println("intTensorPlusDoubleTensor: ${intTensorPlusDoubleTensor.data.toList()}")
        println("doubleTensorPlusDoubleTensor: ${doubleTensorPlusDoubleTensor.data.toList()}")
        println("doubleTensorPlusIntTensor: ${doubleTensorPlusIntTensor.data.toList()}")

        assert(
            intTensorPlusIntTensor.data.contentEquals(
                arrayOf(
                    2.typeInt(), 4.typeInt(), 4.typeInt(),
                    6.typeInt(), 6.typeInt(), 8.typeInt()
                )
            )
        )
        assert(
            intTensorPlusDoubleTensor.data.contentEquals(
                arrayOf(
                    2.0.typeDouble(), 4.0.typeDouble(), 4.0.typeDouble(),
                    6.0.typeDouble(), 6.0.typeDouble(), 8.0.typeDouble()
                )
            )
        )
        assert(
